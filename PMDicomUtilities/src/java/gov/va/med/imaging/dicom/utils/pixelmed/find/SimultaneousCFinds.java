 ...
 *
 */
public class SimultaneousCFinds {

	Vector<CFindInfo> infoList;
	int debugLevel = 0;
	int delay = 0;
	...
	public static void main(String[] args) {
		...
		SimultaneousCFinds finds = new SimultaneousCFinds();
		if(args.length < 1){
	...
	/* (non-Javadoc)
	 * @see gov.va.med.imaging.dicom.utils.pixelmed.find.ISimultaneousCFinds#runThreads()
	 */
	public void runThreads(){
		System.out.println("Running Threads.\n");
		int listSize = this.infoList.size();
		CFindThread[] threads = new CFindThread[listSize];
		
		for(int i=0; i<listSize; i++){
			threads[i] = new CFindThread(this.infoList.get(i));
		}
	...
	private void createInfoList(String configFile) throws ReadFileException, DicomException{
		...
		this.infoList = new Vector<CFindInfo>();
		...
		while((line = parser.getNextTextLine()) != null){
			...
			String data = parser.piece(line, '|', 5);
			if(data.isEmpty()){
	...
	private AttributeList completeAttributeListForCFind(AttributeList identifier)throws DicomException{
		...
		if(!identifier.containsKey(TagFromName.StudyInstanceUID)){
	...
	private String getUsage(){
		String usage = "SimultaneousFinds <configfile> <opt:debug> <opt:delay>\n" +
				"\t<configfile> contains an entry for each new thread.\n" +
				"\t\tEach entry format: ipaddress|port|CalledAET|CallingAET|ElementTag=value~ElementTag=value...|CANCEL|Cancel Delay"+
