...
import gov.va.med.imaging.dicom.utils.pixelmed.simulator.SendDirectoryOfImages;
import gov.va.med.imaging.dicom.utils.pixelmed.simulator.SimulatorInfo;
...
public class CDImporterGUI extends javax.swing.JFrame implements ListEventListener<DicomConfigInfo>{
	 ...
	 * 
	 */
	private static final long serialVersionUID = 9185413230362995698L;
	/**
     * local variables
     */
    ...
    private void initComponents() {
        ...
        jMenuItemDisplay.setEnabled(false);
        ...
        jMenuImportConfig.setEnabled(false);
        ...
        Thread thread = new Thread(new Runnable() {
            ...
            public void run (){
                ...
                if (!UseMWL.isSelected()){
                    ...
                    AttributeList localData = new AttributeList();
                    WorkListEntry wlData = null;
                    try {
                        Attribute pn = new PersonNameAttribute(TagFromName.PatientName);
						pn.addValue(PatientName.getText());
	                    localData.put(pn);
	                    Attribute pid = new LongStringAttribute(TagFromName.PatientID);
	                    pid.addValue(PatientID.getText());
	                    localData.put(pid);
	                    Attribute acc = new ShortStringAttribute(TagFromName.AccessionNumber);
	                    acc.addValue(AccessionNumber.getText());
	                    localData.put(acc);
	                    wlData = new WorkListEntry(localData);
					} catch (DicomException dX) {
			            Logger.getLogger(CDImporterGUI.class.getName()).log(Level.WARNING, null, dX);
					} catch (Exception X) {
			            Logger.getLogger(CDImporterGUI.class.getName()).log(Level.WARNING, null, X);
					}
                    sender = new SendDirectoryOfImages(symInfo,wlData);
                } else {
